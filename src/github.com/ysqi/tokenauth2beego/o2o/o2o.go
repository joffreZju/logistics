...
func (a *O2OAutomatic) NewSingleToken(userID string, group int, w ...http.ResponseWriter) (token *tokenauth.Token, err error) {
	...
	// New token
	token, err = tokenauth.NewSingleToken(userID, group, a.Audience, a.TokenFunc)
	if err != nil {
		return
	}
